...
"""
to run : to run : py -m metrics.Metric8.ml.processing
if there is a lot of None values at the end, re-run this script because sometimes the problem comes from the API
"""



df = pd.read_csv('metrics/Metric8/ml/cleaned_training_data.csv', parse_dates=['date'])
...
df.to_csv('metrics/Metric8/ml/processed_data.csv', index=False)