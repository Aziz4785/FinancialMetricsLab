...
def fetch_stock_data(stocks):
    ...
    for c, stock in enumerate(stocks):
        ...
        if historic_data is not None and "historical" in historic_data:
            ...
            date_to_close,high_dict,low_dict,open_dict,vwap_dict,volume_dict = convert_to_dicts(historic_data["historical"],parse_time=False)
            ...
            #open_df[stock]= convert_to_df(open_dict,'open')
            open_data[stock] =open_dict
            historical_data_for_stock[stock] = date_to_close
            vwap_data[stock] = vwap_dict
            volume_data[stock] = volume_dict
    ...
    return historical_data_for_stock, vwap_data,volume_data,open_data,high_df, low_df,market_cap_dict, revenues_dict, hist_data_df_for_stock,balance_shit_dict,cashflow_dict,estimations_dict,sector_dict
...
def convert_to_dicts(historic_data, parse_time=True,with_vwap_volume=True):
    ...
    for entry in historic_data:
        try:
            ...
            date_to_open[date_obj] = entry['open']
            if with_vwap_volume:
                data_to_vwap[date_obj] = entry['vwap']
                data_to_volume[date_obj] = entry['volume']
        except ValueError as e:
            print(f"Date format error in entry {entry}: {e}")
    return date_to_close, date_to_high, date_to_low, date_to_open,data_to_vwap,data_to_volume
...
def load_models():
    with open('metrics/Metric9/models/RF_w_1_6.pkl', 'rb') as model_file:
        model = pickle.load(model_file)
    with open('metrics/Metric9/models/scaler_1_6.pkl', 'rb') as scaler_file:
        scaler = pickle.load(scaler_file)
    with open('metrics/Metric9/models/XGB2_w4_0_6.pkl', 'rb') as model_file:
        model2 = pickle.load(model_file)
    with open('metrics/Metric9/models/scaler_0_6.pkl', 'rb') as scaler_file:
        scaler2 = pickle.load(scaler_file)
    with open('metrics/Metric9/models/XGB2_w4_4_22.pkl', 'rb') as model_file:
        model3 = pickle.load(model_file)
    with open('metrics/Metric9/models/scaler_4_22.pkl', 'rb') as scaler_file:
        scaler3 = pickle.load(scaler_file)
    return model,scaler,model2,scaler2,model3,scaler3
...
def predict_buy(model,scaler,features_for_pred,input_date,symbol,param_from_simul,price_at_date,
    ...
    total_debt = safe_dict_get(balance_features,'totalDebt')
    max_minus_min8M = safe_subtract(max_in_8M,min_in_8M)
    max_minus_min = safe_subtract(max_in_4M,min_in_4M)
    Spread4Mby8M = safe_divide(max_minus_min,max_minus_min8M)
    ...
    evgp = safe_divide(EV,gp)
    ebitda = safe_dict_get(income_features,'ebitda')
    evebitda =safe_divide(EV,ebitda)
    ...
    if(isinstance(sma_100d_dict, list)):
        ...
        sma_10d_2weeks_ago = extract_SMA((pd.to_datetime(input_date) - pd.DateOffset(weeks=2)).date(), sma_10d_dict)
        sma_10d_1mago = extract_SMA((pd.to_datetime(input_date) - pd.DateOffset(months=1)).date(), sma_10d_dict)
        ...
        sma_10d_7mago = extract_SMA((pd.to_datetime(input_date) - pd.DateOffset(months=7)).date(), sma_10d_dict)
    else:
        sma100d = extract_SMA(input_date, sma_100d_dict.get(symbol, None))
        sma50d = extract_SMA(input_date,sma_50d_dict.get(symbol, None))
        sma10d = extract_SMA(input_date,sma_10d_dict.get(symbol, None))
        sma_50w = extract_SMA(input_date,sma_50w_dict.get(symbol, None))
        sma_200d = extract_SMA(input_date,sma_200d_dict.get(symbol, None))
        sma_10d_2weeks_ago = extract_SMA((pd.to_datetime(input_date) - pd.DateOffset(weeks=2)).date(), sma_10d_dict[symbol])
        sma_10d_1mago = extract_SMA((pd.to_datetime(input_date) - pd.DateOffset(months=1)).date(), sma_10d_dict[symbol])
        ...
        sma_10d_7mago= extract_SMA((pd.to_datetime(input_date) - pd.DateOffset(months=7)).date(), sma_10d_dict[symbol])

    deriv_3m = safe_divide(safe_subtract(price_at_date, sma_10d_3mago),3)
    deriv_1m = safe_divide(safe_subtract(price_at_date, sma_10d_1mago),1)
    deriv_2m = safe_divide(safe_subtract(price_at_date,sma_10d_2months_ago),2)
    feature_calculations = {
        'gp_ratio': lambda: param_from_simul,
        'balance_lag_days': lambda: safe_dict_get(balance_features,'bal_lag_days'),
        'income_lag_days': lambda: safe_dict_get(income_features,'inc_lag_days'),
        'month': lambda:pd.to_datetime(input_date).month,
        'curr_est_eps': lambda: curr_est_eps,
        'netIncome': lambda: netincome,
        'ebitda': lambda: ebitda,
        'total_debt': lambda: total_debt,
        ...
        'var_sma50D': lambda: safe_divide(safe_subtract(price_at_date, sma50d),sma50d),
        'netDebt': lambda: netDebt,
        ...
        'deriv_2w': lambda:safe_divide(safe_subtract(price_at_date,sma_10d_2weeks_ago),0.5),
        'deriv_2m': lambda:deriv_2m,
        'deriv_3m': lambda: deriv_3m,
        ...
        'deriv2_1_3m': lambda:  safe_divide(safe_subtract(deriv_3m, deriv_1m),2),
        'deriv2_2_3m' : lambda: safe_subtract(deriv_3m, deriv_2m),
        'deriv_min8M':lambda: safe_divide(safe_subtract(price_at_date,min_in_8M),min8M_lag),
        'sma_10d_to_sma_50d_ratio': lambda : safe_subtract(safe_divide(sma10d, sma50d), 1),
        ...
        'price_to_sma_100d_ratio':lambda :safe_subtract(safe_divide(price_at_date, sma100d), 1),
        ...
        'max_in_1M': lambda:max_in_1M,
        'min_in_4M' : lambda: min_in_4M,
        'min8M_lag': lambda: min8M_lag,
        'Spread4Mby8M' : lambda: Spread4Mby8M,
        'max_in_2W': lambda: max_in_2W,
        'max_minus_min8M': lambda: max_minus_min8M,
        'markRevRatio': lambda: markRevRAtio ,
        'eps_growth': lambda: eps_growth,
        'pe': lambda: pe,
        'peg': lambda: peg,
        'EVGP': lambda: evgp,
        'combined_valuation_score' :lambda: safe_divide(safe_add(safe_add(safe_add(pe, peg),safe_add(evebitda,evgp)),markRevRAtio),5),
        'EVRevenues': lambda: safe_divide(EV,revenues),
        'EVEbitdaRatio': lambda: evebitda,
        'fwdPriceTosale': lambda:fwdPriceTosale,
        'fwdPriceTosale_diff': lambda: safe_subtract(fwdPriceTosale,markRevRAtio),
        ...
        '7M_return':lambda: safe_divide(safe_subtract(price_at_date,sma_10d_7mago),sma_10d_7mago),
        '1Y_6M_growth': lambda: safe_divide(safe_subtract(sma_10d_6mago,sma_10d_11mago),sma_10d_11mago),
        '3M_2M_growth': lambda: safe_divide(safe_subtract(sma_10d_2months_ago,sma_10d_3mago),sma_10d_3mago),
    }
...
def extract_multiple_SMAs(base_date: Union[str, datetime, pd.Timestamp], sorted_data: List[dict]) -> Dict[str, float]:
    ...
    labels = ['sma_10d_1weeks_ago','sma_10d_2weeks_ago','sma_10d_1months_ago','sma_10d_2months_ago','sma_10d_3months_ago','sma_10d_4months_ago','sma_10d_5months_ago','sma_10d_6months_ago','sma_10d_7months_ago', 'sma_10d_11months_ago']
    date_list = [base_date - pd.DateOffset(weeks=1),base_date - pd.DateOffset(weeks=2),base_date - pd.DateOffset(months=1),base_date - pd.DateOffset(months=2),base_date - pd.DateOffset(months=3), base_date - pd.DateOffset(months=4),base_date - pd.DateOffset(months=5), base_date - pd.DateOffset(months=6), base_date - pd.DateOffset(months=7), base_date - pd.DateOffset(months=11)]
...
def calculate_div_payout_ratio(input_date,cashflow_data,income_data):
    ...
    div_paid = safe_dict_get(cashflow_data,'dividendsPaid')
    net_income = safe_dict_get(income_data,'netIncome')
...
def calculate_ps_ratio(market_cap,income_features):
    if income_features is None :
        return None
    revenues = income_features['revenue']
    if revenues is None or revenues <=0 or market_cap is None or market_cap <=0:
        return None
...
def extract_total_asset_from_features(balance_features):
    if balance_features is None:
        return None
    totalAssets=balance_features['totalAssets']
    return totalAssets
...
def calculate_evebitda_from_features(random_date,mc,balance_features,income_features):
    ...
    if ebitda is None or ebitda==0 or EV is None or EV==0:
        return None
    return EV/ebitda
