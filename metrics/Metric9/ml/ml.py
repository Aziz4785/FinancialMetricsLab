...
"""
to run : py -m metrics.Metric9.ml.ml
...
['gp_ratio', 'sma_10d', 'month', 'RnD_expenses', 'GP', 'netIncome',
       ...
       ['sma_10d_5months_ago', 'month', 'ebitda', 'cashcasheq', 'netDebt', 'curr_est_eps', 'max_minus_min', 'dist_min8M_4M', 'max_minus_min8M', 'Spread4Mby8M', 'debtToPrice', 'peg', 'PS_to_PEG', 'dividend_payout_ratio', 'EVEbitdaRatio', 'fwdPriceTosale_diff', 'combined_valuation_score'] ,
       [ 'sma_10d_5months_ago', 'sma_10d_11months_ago', 'income_lag_days', 'marketCap', 'EV', 'max_minus_min', 'min8M_lag', 'Spread4Mby8M', 'pe', 'peg', 'EVEbitdaRatio', 'EVGP', 'EVRevenues', 'fwdPriceTosale', 'fwdPriceTosale_diff', 'deriv_2w', 'deriv_2m', 'deriv2_1_3m', 'deriv2_2_3m', 'deriv_4m', 'deriv_5m', 'combined_valuation_score', 'sma10_yoy_growth'],
       ['sma_10d_6months_ago', 'marketCap', 'balance_lag_days', 'max_minus_min', 'min8M_lag', 'Spread4Mby8M', 'markRevRatio', 'peg', 'EVEbitdaRatio', 'EVGP', 'fwdPriceTosale', 'fwdPriceTosale_diff', 'deriv_2w', 'deriv_2m', 'deriv2_1_3m', 'deriv2_2_3m', 'deriv_4m', 'deriv_5m', '1y_return', 'combined_valuation_score'],
       ['price', 'eps_growth', 'sma_100d', 'month', 'GP', 'netIncome', 'marketCap', 'cashcasheq', 'EV', 'dist_max8M_4M', 'markRevRatio', 'netDebtToPrice', 'dividend_payout_ratio', 'EVGP', 'fwdPriceTosale_diff', 'deriv_6m', 'deriv_min8M'],
]
...
models = {
    ...
    'RF4a_w': RandomForestClassifier(n_estimators=100,max_features=None,min_samples_leaf=1,min_samples_split=2),
    ...
    'RF5_w': RandomForestClassifier(n_estimators=200,max_features='sqrt',min_samples_leaf=1,min_samples_split=2),
    'RF5_w4': RandomForestClassifier(n_estimators=200,max_features='sqrt',min_samples_leaf=1,min_samples_split=2),
    ...
    'RF11_w': RandomForestClassifier(n_estimators= 300, max_features='sqrt', min_samples_leaf= 1, min_samples_split= 2,max_depth= 10),
    ...
    'XGB1_w':XGBClassifier(n_estimators=100),
    'XGB2':XGBClassifier(n_estimators=200),
    'XGB2_w4':XGBClassifier(n_estimators=200),
    ...
    'XGB5_w':XGBClassifier(n_estimators=300,colsample_bytree= 0.9,gamma=0.1,learning_rate=0.1,max_depth=7,min_child_weight=1,subsample=0.7),
    ...
    'XGB11':XGBClassifier(n_estimators=200,colsample_bytree= 0.9,gamma=0.1,learning_rate=0.3,max_depth=7,min_child_weight=1,subsample=0.9),
     'XGB12':XGBClassifier(n_estimators=200,colsample_bytree= 0.9,gamma=0,learning_rate=0.3,max_depth=7,min_child_weight=1,subsample=0.8),
      ...
      'XGB14':XGBClassifier(n_estimators=200,colsample_bytree=  0.9, gamma= 0, learning_rate=0.3, max_depth= 8,min_child_weight=1,subsample=0.6),
      'XGB14_w':XGBClassifier(n_estimators=200,colsample_bytree=  0.9, gamma= 0, learning_rate=0.3, max_depth= 8,min_child_weight=1,subsample=0.6),
...
for i, subset in enumerate(feature_subsets):
    ...
    for name, model in models.items():
       ...
       elif name.endswith("_w4"):
              weights = np.where(y_train == 0, 4, 1)
              model.fit(X_train_scaled, y_train, sample_weight=weights)
       else:
       ...
       if (precision>0.95 and specificity>0.95) or (precision>0.91 and specificity>0.98) or specificity>0.99:
