...
"""
to run : to run : py -m metrics.Metric9.ml.processing
if there is a lot of None values at the end, re-run this script because sometimes the problem comes from the API
"""



df = pd.read_csv('metrics/Metric9/ml/cleaned_training_data.csv', parse_dates=['date'])
...
df['open_price']= df.apply(lambda row :  extract_stock_price_at_date(row['date'],open_dict[row['symbol']]), axis=1)
df['vwap']= df.apply(lambda row :  extract_stock_price_at_date(row['date'],vwap_dict[row['symbol']]), axis=1)

df['volume']= df.apply(lambda row :  extract_stock_price_at_date(row['date'],volume_dict[row['symbol']]), axis=1)

df['price_to_vwap'] = df.apply(lambda row : safe_multiply(100,safe_divide(row['price'],row['vwap'])), axis=1)
...
df['maxPercen_2W'] = df.apply(lambda row: 
    safe_divide((row['max_in_2W'] - row['price']),row['price']), 
    axis=1)
df['GPRatio'] = df.apply(lambda row: 
    calculate_growthpotential_ratio(
                row['RnD_expenses'],
                row['c_freeCashFlow'],
                row['revenues']
            ), 
...
df['open_price_to_now'] = df.apply(lambda row: 
    safe_divide(safe_subtract(row['price'],row['open_price']),row['open_price']), 
    axis=1)
...
df['deriv_2w'] = df.apply(lambda row: 
    (row['price'] - row['sma_10d_2weeks_ago'])/0.5, 
    axis=1)
...
df['diff_deriv_1m1w'] = df.apply(lambda row: 
    safe_subtract(row['deriv_1m'],row['deriv_1w']), 
    axis=1)
...
df['deriv2_1_2m'] = df.apply(lambda row: 
    safe_subtract(row['deriv_2m'], row['deriv_1m'])/1, 
    axis=1)

df['deriv_3m'] = df.apply(lambda row: 
    safe_divide(safe_subtract(row['price'], row['sma_10d_3months_ago']),3), 
    axis=1)

df['deriv2_1_3m'] = df.apply(lambda row: 
    safe_subtract(row['deriv_3m'], row['deriv_1m'])/2, 
    axis=1)


df['deriv2_2_3m'] = df.apply(lambda row: 
    safe_subtract(row['deriv_3m'], row['deriv_2m'])/1, 
    axis=1)
...
df.to_csv('metrics/Metric9/ml/processed_data.csv', index=False)