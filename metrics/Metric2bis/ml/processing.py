...
"""
to run : to run : py -m metrics.Metric2bis.ml.processing
if there is a lot of None values at the end, re-run this script because sometimes the problem comes from the API
"""



df = pd.read_csv('C:/Users/aziz8/Documents/FinancialMetricsLab/metrics/Metric2bis/ml/cleaned_training_data.csv', parse_dates=['date'])
...
df['ebitdaMargin'] = df.apply(lambda row:
                              safe_divide(row['ebitda'],row['revenue']))
