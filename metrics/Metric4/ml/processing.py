...
"""
to run : to run : py -m metrics.Metric4.ml.processing
...
df = pd.read_csv('C:/Users/aziz8/Documents/FinancialMetricsLab/metrics/Metric4/ml/cleaned_training_data.csv', parse_dates=['date'])
...
"""
df['sma_10d_1weeks_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(weeks=1)).date(), sma_10d_dict[row['symbol']]), axis=1
)

df['sma_10d_1months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=1)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_2months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=2)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_3months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=3)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_4months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=4)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_5months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=5)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_6months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=6)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_7months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=7)).date(), sma_10d_dict[row['symbol']]), axis=1
)
df['sma_10d_11months_ago'] = df.apply(
    lambda row: extract_SMA((pd.to_datetime(row['date']) - pd.DateOffset(months=11)).date(), sma_10d_dict[row['symbol']]), axis=1
)
"""
...
#df['ebitda'] = df.apply(lambda row: extract_EBITDA(row['date'], income_dict[row['symbol']]), axis=1)
results = df.apply(
...
# df['curr_est_eps']=df.apply(lambda row: extract_Field(row['date'], estimations_dict[row['symbol']],'estimatedEpsAvg'), axis=1)
# df['future_est_eps']=df.apply(lambda row: extract_Field(row['date'], estimations_dict[row['symbol']],'future_estimated_eps'), axis=1)
results3 = df.apply(
...
df['var_price_minmax8M'] = df.apply(lambda row: 
    ...
    else (row['price'] - row['min_in_8M'])/row['max_minus_min8M'], 
    axis=1)
...
df['debtToPrice'] = df.apply(lambda row: 
    ...
    else row['total_debt']/row['marketCap'], 
    axis=1)
...
df['eps_growth'] = df.apply(lambda row: 
    ...
    else (row['future_est_eps']-row['curr_est_eps'])/row['curr_est_eps'], 
    axis=1)
...
df['EVGP'] = df.apply(lambda row: 
    None if row['GP'] is None or row['GP'] == 0 
    else row['EV']/row['GP'], 
    axis=1)
...
df['fwdPriceTosale_diff'] = df.apply(lambda row: 
    row['fwdPriceTosale']-row['markRevRatio'], 
    axis=1)
...
df['var_sma50D'] = df.apply(lambda row: 
    None if row['sma_50d'] is None or row['sma_50d'] == 0 
    else (row['price'] - row['sma_50d'])/row['sma_50d'], 
    axis=1)
df['var_sma100D'] = df.apply(lambda row: 
    None if row['sma_100d'] is None or row['sma_100d'] == 0 
    else (row['price'] - row['sma_100d'])/row['sma_100d'], 
    axis=1)
df['var_sma10D'] = df.apply(lambda row: 
    None if row['sma_10d'] is None or row['sma_10d'] == 0 
    else (row['price'] - row['sma_10d'])/row['sma_10d'], 
    axis=1)

df['var_sma10D_50D'] = df.apply(lambda row: 
    None if row['sma_50d'] is None or row['sma_50d'] == 0 
    else (row['sma_10d'] - row['sma_50d'])/row['sma_50d'], 
    axis=1)

df['var_sma50D_100D'] = df.apply(lambda row: 
    None if row['sma_100d'] is None or row['sma_100d'] == 0 
    else (row['sma_50d'] - row['sma_100d'])/row['sma_100d'], 
    axis=1)

df['var_sma50D_200D'] = df.apply(lambda row: 
    None if row['sma_200d'] is None or row['sma_200d'] == 0 
    else (row['sma_50d'] - row['sma_200d'])/row['sma_200d'], 
    axis=1)
